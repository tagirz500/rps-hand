...
const held=[...s.eye];s.update(5000,.016);near(s.eye[2],held[2],.003);
...
assert.ok(view.physicalRoll>=.3&&view.physicalRoll<.42);near(view.physicalYaw,0);near(view.physicalPitch,0);
const predictedRoll=view.physicalRoll;view.update(5000,.016);near(view.physicalRoll,predictedRoll,.02);
const coastView=new ViewPose();coastView.mode='first';coastView.preserveNeutral=true;
coastView.receive({yaw:0,pitch:0},0);coastView.receive({yaw:-.12,pitch:.04},40);coastView.update(40,.016);const turn=coastView.physicalYaw;coastView.update(300,.1);assert.ok(coastView.physicalYaw>turn,'Angular prediction follows the last turn direction');const heldTurn=coastView.physicalYaw;coastView.update(2000,.1);assert.ok(Math.abs(coastView.physicalYaw-heldTurn)<.03,'Angular prediction is bounded');

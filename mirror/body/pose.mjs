...
export function upperBodyPose(image,world) {
  ...
  const hipsTracked=!!(joints.leftHip&&joints.rightHip);
  const hip=hipsTracked?mid(joints.leftHip,joints.rightHip):null;
  if(hip&&(shoulder[1]-hip[1]<.12||shoulder[1]-hip[1]>.9)) return null;
...
export class BodyPose {
  ...
  recenter(mode=this.mode){
    ...
    this.target=null;this.joints={};this.hipsTracked=false;this.signals=null;
  }
  receive(p,now){
    if(!p) return;
    if(now-this.seen>1500&&Number.isFinite(this.seen)) this.recenter();

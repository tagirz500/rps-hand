...
async def run():
    async with async_playwright() as p:
        ...
        # a search must be escapable: E waits, F sees the lobby, E stops, the lobby is gone, E can search again
        E = await player("E", "victory.jpg"); F = await player("F", "victory.jpg")
        await E.click("#online"); await E.click("#quick")
        await E.wait_for_function("document.getElementById('net').textContent.startsWith('lobby')", timeout=90000)
        print("E searching:", await E.inner_text("#net"), "| button:", await E.inner_text("#quick"))
        await F.click("#online"); await F.click("#refresh"); await asyncio.sleep(4)
        print("F sees:", (await F.inner_text("#rooms")).replace(chr(10), " ")[:44])
        await E.click("#quick"); await asyncio.sleep(1)
        await F.click("#refresh"); await asyncio.sleep(4)
        gone = "Lobby" not in (await F.inner_text("#rooms"))
        print("E stopped:", await E.inner_text("#net"), "| button:", await E.inner_text("#quick"), "| lobby gone for F:", gone)
        await E.click("#quick")
        await E.wait_for_function("document.getElementById('net').textContent.startsWith('lobby')", timeout=90000)
        print("E can search again:", await E.inner_text("#net"))
